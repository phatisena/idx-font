    ...
    export function setCharecter(gid: number = 0, glyph: string = "", imgi: Image = image.create(5, 8), notmove: boolean = false, onthechar: boolean = false, inchar: boolean = false , bcol: number = 0, scol: number = 0, mcol: number = 0, ncol: number = 0) {
        ...
        if (notmove) {
            ...
            for (let yh = 0; yh < imgi.height; yh++) {
                si = 0
                for (let xw = 0; xw < imgi.width; xw++) {
                    if (imgi.getPixel(xw, yh) != 0 || (scwidt && imgi.getPixel(xw, yh + 1) != 0)) { si += 1 }
                }
                if (scnwidt) {
                    if (scwidt) {
    ...
    export function SetTextImage(input: string, iwidt: number, tid: number, icol: number = 0, alm: number = 0, debugalm: boolean = false) {
        ...
        for (let currentletter3 = 0; currentletter3 < input.length; currentletter3++) {
            ...
            if (!(ligs[tid].indexOf(input.charAt(currentletter3)) < 0)) {
                ...
                if (ligsubw[tid][ligs[tid].indexOf(input.charAt(Math.min(currentletter3 + 1, input.length - 1)))] > 0 && Math.abs(ligsubw[tid][ligs[tid].indexOf(input.charAt(Math.min(currentletter3 + 1, input.length - 1)))] - ligwidth[tid][ligs[tid].indexOf(input.charAt(Math.min(currentletter3 + 1, input.length - 1)))]) > 0) {
                ...
                if (ligwidth[tid][ligs[tid].indexOf(input.charAt(currentletter3))] == 0 && Math.abs(ligdir[tid][ligs[tid].indexOf(input.charAt(currentletter3))]) > 0) {
                    if (ligdir[tid][ligs[tid].indexOf(input.charAt(currentletter3))] < 0) {
                        hih = 0;
                        while ((ImgOverlapImg(rimg, limg, curwidt, hih, 0, 1)) || (ImgOverlapImg(rimg, limg, curwidt, hih + 1, 0, 1))) {
                            hih += 1
                        }
                        wie = 0;
                        while (!((ImgOverlapImg(rimg, limg, (curwidt) - wie, hih, 0, 1)) && !(ImgOverlapImg(rimg, limg, (curwidt) - wie, hih + 1, 0, 1)))) {
                            hih += 1
                        }
                        ...
                        if ((ImgOverlapInNum(rimg, limg, curwidt - letterspace, hih, -1, 0, 1))) {
                            while ((ImgOverlapImg(rimg, limg, (curwidt - letterspace) - (wie + 1), hih, -1, 0)) || (ImgOverlapImg(rimg, limg, (curwidt) - (wie + 1), hih, -1, 0))) {
                                wie += 1
                            }
                            while ((ImgOverlapImg(rimg, limg, (curwidt) - wie, hih, 0, 1)) || (ImgOverlapImg(rimg, limg, (curwidt) - wie, hih + 1, 0, 1))) {
                                hih += 1
            ...
            if (!(ligs[tid].indexOf(input.charAt(currentletter3)) < 0)) {
                ...
                if (ligwidth[tid][ligs[tid].indexOf(input.charAt(currentletter3))] == 0 && Math.abs(ligdir[tid][ligs[tid].indexOf(input.charAt(currentletter3))]) > 0) {
                    if (ligdir[tid][ligs[tid].indexOf(input.charAt(currentletter3))] < 0) {
                        ...
                        sc = 1; wie = 0;
                        if ((ImgOverlapInNum(rimg, limg, curwidt - letterspace, hih, -1, 0, 1))) {
                            while ((ImgOverlapImg(rimg, limg, (curwidt) - (wie + 1), hih, -1, 0)) || (ImgOverlapImg(rimg, limg, (curwidt) - (wie + 1), hih, -1, 0))) {
                                wie += 1
                            }
                            while ((ImgOverlapImg(rimg, limg, (curwidt) - wie, hih, 0, 1)) || (ImgOverlapImg(rimg, limg, (curwidt) - wie, hih + 1, 0, 1))) {
                                hih += 1
...
idxfont.SetupPresetFont(idxfont.tempfont.MainFont, 0)
scene.setBackgroundColor(1)
let mySprite = sprites.create(idxfont.SetTextImage("สวัสดี", 160, 0), SpriteKind.Player)